...
# display a msg box and go on if OK is clicked
def msg():
...
def topwin():
    top = Toplevel()
    top.title("Denomination Calculators")
    top.configure(bg='light grey')
    top.geometry("600x350+50+50")

    label = Label(top, text="Enter Total Amount", bg='light grey')
    entry = Entry(top)
    lbl = Label(top, text="Here are number of notese for each denomination",bg='light grey')
